(function (JGS, $, undefined) {
  ...
  /**
   ...
   @constructor
   */
  JGS.Demo7Page = function (pageCfg) {
   ...
   * @method
   */
  JGS.Demo7Page.prototype.init = function () {
  ...
  JGS.Demo7Page.prototype._setupRangeButtons = function () {
    var self = this;
   ...
   * @private
   */
  JGS.Demo7Page.prototype._setupRangeMouseHandling = function () {
    var self = this;
   ...
   * triggered. We use a global JGS.Demo7Page.isGlobalPanInteractionHandlerInstalled flag to make sure we only install
   * the global handler once.
   *
   * @method _setupPanInteractionHandling
   * @private
   */
  JGS.Demo7Page.prototype._setupPanInteractionHandling = function () {

    if (JGS.Demo7Page.isGlobalPanInteractionHandlerInstalled)
      return;
    else
      JGS.Demo7Page.isGlobalPanInteractionHandlerInstalled = true;
   ...
   * @private
   */
  JGS.Demo7Page.prototype._loadNewDetailData = function () {
    this.showSpinner(true);
   ...
   * @private
   */
  JGS.Demo7Page.prototype._onNewGraphData = function (graphData) {
   ...
   * @method drawDygraph
   */
  JGS.Demo7Page.prototype.drawDygraph = function (graphData) {
    ...
    // Load annotations for the current graph data
    var annotations = this.getAnnotations(graphData);
    this.graph.setAnnotations(annotations);
  };

  JGS.Demo7Page.prototype.getAnnotations = function (graphData) {

    var annotations = [];
    var lastDataDateTm = graphData.dyData[graphData.dyData.length - 1][0];


    var workingMom = moment(lastDataDateTm);

    var $activeRangeBtn = this.$rangeBtnsCont.find("button.active");
    var rangeType = "";
    if ($activeRangeBtn) {
      rangeType = $activeRangeBtn.attr('name').replace("range-btn-", "");
    }

    var workingMomDeltaType = "month";
    if (rangeType == "1d") {
      workingMom.startOf('hour');
      workingMomDeltaType = "hour";
    }
    else {
      workingMom.startOf('month');
    }
    ...
    for (var i = 0; i < (12 * 5); i++) {
      ...
      var shortTxt, txt;
      if (rangeType == "1d") {
        shortTxt = workingMom.format("HH");
        txt = workingMom.format("HH:mm");
      }
      else {
        shortTxt = workingMom.format("MM");
        txt = workingMom.format("MMMM");
      }
  ...
  JGS.Demo7Page.prototype._findNearestX = function (dyData, epochMs) {
    // Binary search would be more
    for (var i = dyData.length - 1; i >= 0; i--) {
      if (dyData[i][0].getTime() <= epochMs) {
        return dyData[i][0];
      }
    }
    return null;
  ...
  JGS.Demo7Page.prototype._onDyDrawCallback = function (dygraph, is_initial) {
...
//    this.detailEndDateTm = axisXEndDateTm;
   ...
   * @private
   */
  JGS.Demo7Page.prototype._onDyZoomCallback = function (minDate, maxDate, yRanges) {
   ...
   * @method showSpinner
   */
  JGS.Demo7Page.prototype.showSpinner = function (show) {
    if (show === true) {
